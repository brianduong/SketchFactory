# Video 19 — YouTube Upload — Sheep

## Trạng thái đăng

- YouTube: [x] Đã upload — Private, tự chuyển Public lúc 08-08/2026 20:00 (Asia/Ho_Chi_Minh)
- TikTok: [ ] Chưa đánh dấu
- Facebook: [ ] Chưa đánh dấu
- Instagram: [ ] Chưa đánh dấu

## Title

How to Draw a Sheep in 9 Simple Strokes ✏️

## Description

Learn how to draw a simple sheep in 9 easy strokes.
Follow each step, pause if you need, and try it on paper or a whiteboard.

What should SketchFactory draw next?

Draw anything. The simple way.

#HowToDraw #EasyDrawing #SheepDrawing

## Tags

how to draw a sheep, easy sheep drawing, simple sheep drawing, sheep, animal, farm, easy drawing, drawing for beginners, step by step drawing, 9 strokes, SketchFactory

## Upload Files

- Video: `output/upload-ready/19-sheep.mp4`
- Thumbnail: `output/thumbnails/19-animal-sheep-001.png`
- Subtitles: `output/subtitles/19-animal-sheep-001.srt`

## YouTube Settings

- Audience: No, it's not made for kids
- Age restriction: No
- Video language: English (United States)
- Title and description language: English
- Category: Howto & Style
- Paid promotion: No
- Altered content: No
- License: Standard YouTube License
- Comments: On
- Shorts remixing: On
- Playlist: Simple Drawing Tutorials
- Initial visibility: Private

## YouTube Result

- Video ID: `oZUi5ZOdWeA` — https://youtu.be/oZUi5ZOdWeA
- Upload status: Uploaded 2026-08-02 (bản v2 vẽ nét thuần)
- Privacy: Private, tự chuyển Public lúc 08-08/2026 20:00 (Asia/Ho_Chi_Minh)
- publishAt (UTC): `2026-08-08T13:00:00Z`
- Audience: Not made for kids
- English captions: Uploaded
- Custom thumbnail: Đã đặt
- Playlist: chưa thêm lại sau khi thay bản mới, xem việc treo trong upload-tracker.md

## Final Checklist

- [x] Upload file `19-sheep.mp4`
- [ ] Confirm the opening hook is fully visible on a phone
- [ ] Confirm the thumbnail says `DRAW A SHEEP`
- [ ] Set audience to `No, it's not made for kids`
- [ ] Upload the SRT subtitle file
- [ ] Wait for HD processing and YouTube Checks
- [ ] Watch the full video once on a phone before publishing

## Other Platforms

- Facebook: A sheep anyone can draw—just 9 strokes. Save this and try it on paper or a whiteboard.
- Instagram: Draw a sheep with me in 9 strokes. Simple enough for any beginner. ✏️
- TikTok: Can you draw this sheep in 10 seconds? ✏️ 9 strokes! #EasyDrawing #LearnToDraw #SheepDrawing
